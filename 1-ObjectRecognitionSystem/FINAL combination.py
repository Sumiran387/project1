# line 28,78,80 me nye comments add kare hai unko dekh lijiyega....
...
window.configure(bg="#F778A1")

# Providing Label
design = ['Arial', 10, 'bold']
lbl = Label(window, bg='cyan', fg='black', text="Welcome ", font=design)
...
def create_data():
    ...
    # import cv2     #ye saare imports maine sabse uper hi le liye hai
    # import sys
    # import numpy
    # import os
    ...
    sub_data = input("Enter Your Name : ")  # 'abhijeet'	   #change name
...
def face_recog():
    size = 4       # aur hann bhaiya ye naa jane kyu not used dikha raha.....
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            if prediction[1] < 74:
                cv2.putText(im, '%s' % (names[prediction[0]].strip()), (x + 5, (y + 25) + h), cv2.FONT_HERSHEY_PLAIN,
                            1.5,
                            (20, 185, 20), 2)
...
# Creating buttons,
reg_button = Button(frame, bg='silver', fg='black', text="Create Dataset", command=create_data)
reg_button.grid(row=3, column=0, padx=10)

login_btn = Button(frame, bg='silver', fg='black', text="   Rcognition  ", command=face_recog)
login_btn.grid(row=3, column=1, padx=15)
